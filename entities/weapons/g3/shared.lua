...
SWEP.ViewModel = "models/weapons/v_rif_g3s.mdl"      
...
SWEP.IronSightsPos = Vector(-1.941, -1.2, 0.72)
...
//Primary Attributes
--------------------------
SWEP.Primary.Sound = "weapons_g3/aug-1.wav"        
...
// Adjust the player's aim
	...
	// Shell eject
		...
		local shelldata = EffectData{}
			...
			shelldata:SetAngles(self.Weapon:GetAttachment(ejectattachment).Ang + Angle(20,-5,0))
...
function SWEP:Reload()
	...
	if ( self.Weapon:Clip1() < self.Primary.ClipSize and self.Owner:GetAmmoCount( self.Primary.Ammo ) > 0 ) then
		local shots = self.Primary.ClipSize - self.Weapon:Clip1()
		
		if self.Weapon:Clip1() > 0 then
			self.Primary.ClipSize = clip + 1
			self.Weapon:SetClip1( self.Primary.ClipSize)
			self:TakePrimaryAmmo(shots, self.Primary.Ammo)
